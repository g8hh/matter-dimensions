functions['photonic_1_cost'] = (amt_bought) => {
    ...
    base_amt = base_amt.div(me.upgrades['p52'].get_effect());
...
functions['photonic_1_pow'] = (amt_bought, amt) => {
    ...
    base_exp = me.upgrades['p31'].get_effect();

    var base_pow = base_exp.pow(amt_bought);
    // Photonic Challenge 3: 1st Photonic Dimension is not affected by production boosts
    if (me.challenges['p3'].inC()) return base_pow.div(me.achievement_multiplier);

    // g21: unspent Gravitons boost Photonic dimensions
    if (me.upgrades['g21'].is_active()) base_pow = base_pow.mult(me.upgrades['g21'].get_effect());
    // achievement 35: x10 power to photonic dimensions
    if (me.achievements['35'].complete) base_pow = base_pow.mult(10);
    // n21: bonus to all Photonic Dimensions
    if (me.upgrades['n21'].is_active()) base_pow = base_pow.mult(me.upgrades['n21'].get_effect());
    // v41: bonus to all Photonic Dimensions
    base_pow = base_pow.mult(me.upgrades['v41'].get_effect());
    // v42: bonus to all Photonic Dimensions if matter is over 1e10
    base_pow = base_pow.mult(me.upgrades['v42'].get_effect());
    // p44: bonus to all Photonic Dimensions from photons
    if (me.upgrades['p44'].is_active()) base_pow = base_pow.mult(me.upgrades['p44'].get_effect());
    // v112: x3 power to all Photonic Dimensions
    base_pow = base_pow.mult(me.upgrades['v112'].get_effect());
    // v162: bonus to all Photonic Dimensions
    if (me.upgrades['v162'].is_active()) base_pow = base_pow.mult(me.upgrades['v162'].get_effect());
    // v172: bonus to all Photonic Dimensions
    if (me.upgrades['v172'].is_active()) base_pow = base_pow.mult(me.upgrades['v172'].get_effect());
    // v182: bonus to all Photonic Dimensions based on max matter
    if (me.upgrades['v182'].is_active()) base_pow = base_pow.mult(me.upgrades['v182'].get_effect());
    // v192: Stars boost 1st Photonic Dimension
    if (me.upgrades['v192'].is_active()) base_pow = base_pow.mult(me.upgrades['v192'].get_effect());
    // d101: Inflation boosts 1st and 2nd Photonic Dimensions
    if (me.upgrades['d101'].is_active()) base_pow = base_pow.mult(me.upgrades['d101'].get_effect());
...
functions['photonic_3_pow'] = (amt_bought, amt) => {
    ...
    if (me.achievements['33'].complete) base_pow = base_pow.mult(2);
    // n21: bonus to all Photonic Dimensions
    if (me.upgrades['n21'].is_active()) base_pow = base_pow.mult(me.upgrades['n21'].get_effect());
    // v41: bonus to all Photonic Dimensions
    base_pow = base_pow.mult(me.upgrades['v41'].get_effect());
...
functions['photonic_1_unlock'] = () => {
    ...
    if (!me.upgrades['g20'].is_active()) return false;
...
functions['photonic_2_unlock'] = () => {
    ...
    if (me.dimensions['photonic_1'].amt_bought > 0) return true;
    return false;
}
functions['photonic_3_unlock'] = () => {
    ...
    if (me.dimensions['photonic_2'].amt_bought > 0) return true;
    return false;
}
functions['photonic_4_unlock'] = () => {
    ...
    if (me.dimensions['photonic_3'].amt_bought > 0) return true;
    return false;
}
functions['photonic_5_unlock'] = () => {
    ...
    if (me.dimensions['photonic_4'].amt_bought > 0) return true;
    return false;
}
functions['photonic_6_unlock'] = () => {
    ...
    if (me.dimensions['photonic_5'].amt_bought > 0) return true;
    return false;
}
functions['photonic_7_unlock'] = () => {
    ...
    if (me.dimensions['photonic_6'].amt_bought > 0) return true;
    return false;
}
functions['photonic_8_unlock'] = () => {
    ...
    if (me.dimensions['photonic_7'].amt_bought > 0) return true;
    return false;
}
functions['photonic_9_unlock'] = () => {
    ...
    if (me.dimensions['photonic_8'].amt_bought > 0) return true;
    return false;
}
functions['photonic_10_unlock'] = () => {
    ...
    if (me.dimensions['photonic_9'].amt_bought > 0) return true;
    return false;
}
functions['photonic_11_unlock'] = () => {
    ...
    if (me.dimensions['photonic_10'].amt_bought > 0) return true;
    return false;
}
functions['photonic_12_unlock'] = () => {
    ...
    if (me.dimensions['photonic_11'].amt_bought > 0) return true;
    return false;
}