functions["upg_p11_cost"] = (amt) => {
    ...
    if (me.challenges['n4'].inC()) base_cost = base_cost.pow(2);
    // Neutronic Challenge 4 reward: the cost of all Photonic upgrades is reduced
    base_cost = base_cost.pow(me.challenges['n4'].get_effect());
...
functions["upg_p21_cost"] = (amt) => {
    ...
    if (me.upgrades['g33'].is_active()) base_pow -= 0.25;
...
functions["upg_p31_cost"] = (amt) => {
    let base_pow = 1.5;
    // g33: photonic upgrades past the first row are cheaper
    if (me.upgrades['g33'].is_active()) base_pow -= 0.5;
...
functions["upg_p41_cost"] = (amt) => {
    let base_pow = 3;
    // g33: photonic upgrades past the first row are cheaper
    if (me.upgrades['g33'].is_active()) base_pow -= 0.75;
...
functions["upg_p51_cost"] = (amt) => {
    let base_pow = 9;
    // g33: photonic upgrades past the first row are cheaper
    if (me.upgrades['g33'].is_active()) base_pow -= 1.25;
...
functions["upg_p11_power"] = (amt) => {
    ...
    if (me.upgrades['g43'].is_active()) base_amt = base_amt.mult(2);
    ...
    if (me.challenges['p7'].inC()) base_pow = base_pow.pow(2);
...
functions["upg_p12_power"] = (amt) => {
    ...
    base_pow = base_pow.subtract(1).mult(big(0.9999).pow(me.time_photonic)).add(1);
    return base_pow;
}
...
functions["upg_p23_power"] = (amt) => {
    return big(2).add(big(Math.log10(me.overall_time / 111 + 1)));
}
functions["upg_p24_power"] = (amt) => {
    return big(2).add(big(me.photons.add(1).log(10)).pow(2));
}
...
functions["upg_p32_power"] = (amt) => {
    ...
    return big(me.photonic_resets).pow(base_amt.pow(0.75).div(2)).add(1);
}
functions["upg_p33_power"] = (amt) => {
    return big(1);
}
functions["upg_p34_power"] = (amt) => {
    return me.photons.pow(0.7).min(me.photons.add(1).log(10).pow(4).mult(1e3)).add(2);
}
...
functions["upg_p44_power"] = (amt) => {
    return big(1).add(big(me.photons.add(1).log(10)).pow(3));
}
...
functions["upg_p31_unlock"] = () => {
    ...
    if (me.upgrades['n04'].is_active()) return true;
    return false;
}
...
functions["upg_p41_unlock"] = () => {
    ...
    if (me.upgrades['v82'].is_active()) return true;
    return false;
}
...
functions["upg_p51_unlock"] = () => {
    ...
    if (me.milestones['a05_1'].is_active()) return true;
    return false;
}
...
functions["upg_p21_available"] = () => {
    ...
    if (me.challenges['p5'].inC()) return false;
    // Vacuumic Challenge 7: repeatable upgrades are not available
    if (me.challenges['v7'].inC()) return false;
    return true;
}
