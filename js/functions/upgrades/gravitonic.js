functions["upg_g01_cost"] = (amt) => {
    ...
    if (me.challenges['n5'].inC()) base_cost = base_cost.add(1);
    // Neutronic Challenge 5 reward: cheap Gravitonic upgrades are discounted to 1.
    if (base_cost.lt(me.challenges['n5'].get_effect())) base_cost = big(1);
...
functions["upg_g12_cost"] = (amt) => {
    ...
    if (me.challenges['g6'].completed) base_amt = base_amt * 0.9;
...
functions["upg_g11_power"] = (amt) => {
    let base_gravitons = me.gravitons;
    // n06: gravitonic upgrades are based on max gravitons
    base_gravitons = me.upgrades['n06'].get_effect();
    // Gravitonic Challenge 8: gravitonic upgrades gain power in first 30 seconds
    if (me.challenges['g8'].inC() && me.time_passed < 30000) base_gravitons = base_gravitons.mult(me.time_passed / 30000);
    // Gravitonic Challenge 8 reward: gravitonic upgrades are 8% more powerful in the first 30 seconds
    if (me.challenges['g8'].completed && me.time_passed < 30000) base_gravitons = base_gravitons.mult(1.08);
    // Gravitonic Challenge 1 reward: gravitonic upgrades in top-left block are 10% more powerful
    if (me.challenges['g1'].completed) base_gravitons = base_gravitons.mult(1.1);
...
functions["upg_g12_power"] = (amt) => {
    if (amt == 0) return me.upgrades['g10'].get_effect();

    let base_amt = amt;
    // Gravitonic Challenge 8: gravitonic upgrades gain power in first 30 seconds
    if (me.challenges['g8'].inC() && me.time_passed < 30000) base_amt = base_amt * (me.time_passed / 30000);
    // Gravitonic Challenge 8 reward: gravitonic upgrades are 8% more powerful in the first 30 seconds
    if (me.challenges['g8'].completed && me.time_passed < 30000) base_amt = base_amt * 1.08;
    // Gravitonic Challenge 1 reward: gravitonic upgrades in top-left block are 10% more powerful
    if (me.challenges['g1'].completed) base_amt = base_amt * 1.1;
...
functions["upg_g21_power"] = (amt) => {
    ...
    if (me.challenges['g2'].completed) base_gravitons = base_gravitons.mult(2);
...
functions["upg_g22_power"] = (amt) => {
    return me.light.add(1).pow(0.5);
}
functions["upg_g23_power"] = (amt) => {
    if (amt == 0) return big(1);
    return power_light_production(me);
}
functions["upg_g24_power"] = (amt) => {
    if (me.challenges['p4'].inC()) return big(1);
    return power_light_time(me);
}
...
functions["upg_g32_power"] = (amt) => {
    if (amt == 0) return me.upgrades['g30'].get_effect();
...
functions["upg_g10_unlock"] = () => {
    ...
    if (me.challenges['g1'].inC()) return false;

    return me.upgrades['g01'].amt > 0;
}
functions["upg_g11_unlock"] = () => {
    // Gravitonic Challenge 1: upper-left block is disabled
    if (me.challenges['g1'].inC()) return false;

    return me.upgrades['g10'].amt > 0;
}
...
functions["upg_g13_unlock"] = () => {
    ...
    return me.upgrades['g10'].amt > 0 && me.challenges['p1'].completed;
}
functions["upg_g14_unlock"] = () => {
    // Gravitonic Challenge 1: upper-left block is disabled
    if (me.challenges['g1'].inC()) return false;

    return me.upgrades['g10'].amt > 0 && me.challenges['p2'].completed;
}
functions["upg_g20_unlock"] = () => {
    // Gravitonic Challenge 2: upper-right block is disabled
    if (me.challenges['g2'].inC()) return false;
...
functions["upg_g23_unlock"] = () => {
    ...
    return me.upgrades['g20'].amt > 0 && me.challenges['p3'].completed;
}
functions["upg_g24_unlock"] = () => {
    // Gravitonic Challenge 2: upper-right block is disabled
    if (me.challenges['g2'].inC()) return false;

    return me.upgrades['g20'].amt > 0 && me.challenges['p4'].completed;
}
functions["upg_g30_unlock"] = () => {
    // Gravitonic Challenge 3: lower-left block is disabled
    if (me.challenges['g3'].inC()) return false;
...
functions["upg_g33_unlock"] = () => {
    ...
    return me.upgrades['g30'].amt > 0 && me.challenges['p5'].completed;
}
functions["upg_g34_unlock"] = () => {
    // Gravitonic Challenge 3: lower-left block is disabled
    if (me.challenges['g3'].inC()) return false;

    return me.upgrades['g30'].amt > 0 && me.challenges['p6'].completed;
}
...
functions["upg_g43_unlock"] = () => {
    ...
    return me.upgrades['g40'].amt > 0 && me.challenges['p7'].completed;
}
functions["upg_g44_unlock"] = () => {
    // Gravitonic Challenge 4: lower-right block is disabled
    if (me.challenges['g4'].inC()) return false;

    return me.upgrades['g40'].amt > 0 && me.challenges['p8'].completed;
}
...
functions["upg_g01_buy"] = (amt) => {
    ...
    if (!me.challenges['p8'].inC()) me.photons = me.photons.add(me.upgrades['g01'].get_effect()).round().min(me.challenge_strength_4);
}